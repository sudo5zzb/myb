...
public class MerkleTree {
    ...
    private void buildTree() {
        ...
        for (String data : datas) {
            String hash = HashUtil.getSHA2HexValue(data);
        ...
        while (tmpNodes.size() > 1) {
            ...
            for (; i < nodes.size(); ) {
                ...
                if (i != nodes.size()) {
                    ...
                    parentNode = new Node(Lists.newArrayList(left, right), HashUtil.getSHA2HexValue(left.hash + right.hash));
                    i++;
                } else {
                    parentNode = new Node(Lists.newArrayList(left), HashUtil.getSHA2HexValue(left.hash));
                }
